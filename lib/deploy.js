    ...
    */
    ...
    Deploy.prototype.loadYAML = function() {
      ...
      return fs.readFile("dploy.yaml", function(error, data) {
        var code, key, yaml;
        if (error) {
          return console.log("Error:".bold.magenta, "The file \"dploy.yaml\" could not be found.");
          process.exit(code = 0);
        }
        ...
        if (!_this.config) {
          return console.log("Error:".bold.magenta, "We couldn't find the settings for " + ("" + _this.server).bold.magenta);
          process.exit(code = 0);
        }
    ...
    */
    ...
    Deploy.prototype.checkPassword = function(callback) {
      ...
      prompt.get([
        {
          name: "password",
          description: "Enter the password for ".magenta + ("" + this.config.host + ":").underline.bold.magenta,
          requimagenta: true,
          hidden: true
        }
    ...
    */
    ...
    Deploy.prototype.checkBranch = function() {
      ...
      return exec("git rev-parse --abbrev-ref HEAD", function(error, stdout, stderr) {
        ...
        if (error) {
          return console.log("An error occurmagenta when retrieving the current branch.".bold.magenta, error);
        }
        ...
        console.log("Error: ".magenta.bold + "You are not allowed to deploy from ".magenta + ("" + currentBranch).bold.underline.magenta + " to ".magenta + ("" + _this.server).bold.underline.magenta);
    ...
    */
    ...
    Deploy.prototype.setupGit = function() {
      ...
      return exec("git log --pretty=format:%H -n 1", function(error, stdout, stderr) {
        if (error) {
          return console.log("An error occurmagenta when retrieving the local hash.".bold.magenta, error);
        }
    ...
    Deploy.prototype.setupServer = function() {
      ...
      this.connection.failed.add(function() {
        if (!_this.isConnected) {
          return console.log("Connection failed.".bold.magenta);
        }
      });
      ...
      return this.connection.get(remotePath, function(error, data) {
        if (error) {
          fs.writeFile(_this.revisionPath, _this.local_hash, function(error) {
            if (error) {
              return console.log("Error creating revision file at:".magenta, ("" + _this.revisionPath).magenta.bold.underline, error);
            }
            return _this.addAll();
          });
          return;
    ...
    Deploy.prototype.checkDiff = function(old_rev, new_rev) {
      ...
      if (old_rev === new_rev) {
        if (this.config.include) {
          ...
          console.log("No diffs between local and remote :)".cyan);
      ...
      return exec("git diff --name-status " + old_rev + " " + new_rev, {
        ...
        if (error) {
          return console.log(("An error occurmagenta when retrieving the 'git diff --name-status " + old_rev + " " + new_rev + "'").bold.magenta, error);
        }
        if (!_this.catchup) {
          files = stdout.split("\n");
    ...
    */
    ...
    Deploy.prototype.addAll = function() {
      ...
      return exec("git ls-tree -r --name-only HEAD", {
        ...
        if (error) {
          return console.log("An error occurmagenta when retrieving 'git ls-tree -r --name-only HEAD'".bold.magenta, error);
        }
        if (!_this.catchup) {
          files = stdout.split("\n");
          for (_i = 0, _len = files.length; _i < _len; _i++) {
    ...
    */
    ...
    Deploy.prototype.askBeforeUpload = function() {
      ...
      if (this.toDelete.length) {
        console.log("Files that will be deleted:".bold.magenta);
        _ref = this.toDelete;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          file = _ref[_i];
          console.log("[ ? ]".grey, ("" + file.remote).magenta);
        }
      }
      if (this.toUpload.length) {
        console.log("Files that will be uploaded:".bold.cyan);
        _ref1 = this.toUpload;
        for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
          file = _ref1[_j];
          remoteFile = this._normalize(this.config.path.remote + file.remote);
          console.log("[ ? ]".cyan, ("" + file.name).cyan, ">".green, ("" + remoteFile).cyan);
        }
      }
      prompt.start();
      return prompt.get([
        {
          name: "answer",
          pattern: /y|n|Y|N/,
          description: "Are you sure you want to upload those files?".bold.magenta + " (Y/n)",
          message: "The answer should be YES (y) or NO (n)."
        }
      ], function(error, result) {
        if (result === undefined) {
          console.log("Stop by user.".magenta);
          process.exit();
          return false;
        }
        ...
        if (result.answer.toLowerCase() === "y" || result.answer.toLowerCase() === "") {
          ...
          console.log("Upload aborted by the user.".magenta);
          return _this.removeConnections(false);
        }
      });
    };
    ...
    */
    ...
    Deploy.prototype.hasFilesToUpload = function() {
      if (this.toUpload.length === 0 && this.toDelete.length === 0) {
        console.log("No files to upload".cyan);
    ...
    Deploy.prototype.uploadItem = function(connection, item) {
      ...
      // if (fs.statSync(item.name)) {
        return connection.upload(item.name, remote_path, function(error) {
          if (error) {
            console.log("[ + ]".cyan, ("Fail uploading file " + item.name + ":").magenta, error);
            item.started = false;
            item.completed = false;
          } else {
            console.log("[ + ]".cyan + (" File uploaded: " + item.name + ":").cyan);
            item.completed = true;
          }
          return _this.nextOnQueue(connection);
        });
      } else {
        console.log("[ + ]".magenta, ("WTF? " + item.name + ":").magenta);
        item.completed = true;
    ...
    */
    ...
    Deploy.prototype.deleteItem = function(connection, item) {
      ...
      return connection["delete"](remote_path, function(error) {
        if (error) {
          console.log("[ × ]".grey, ("Fail deleting file " + remote_path + ":").magenta);
        } else {
